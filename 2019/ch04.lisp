...
(defvar *jan-ken-pon* '((rock (scissors paper))
                        (paper (rock scissors))
                        (scissors (paper rock))))
(defun play (first second)
  (let ((results (assoc first *jan-ken-pon*)))
    (if (null results)
        (error "Invalid move ~A." first)
        (destructuring-bind (win lose) (second results) 
          (cond ((eq first second) 'tie)
                ((eq second win) 'first-wins)
                ((eq second lose) 'second-wins)
                (t (error "Invalid move ~A." second)))) )))
